...
class generateDataset(Dataset):
    def __init__(self, data, 
        ...
        self.spec_list = self.process_spec(spec_len, data["spectra"], aug_mode=aug_mode, aug_num=aug_num, 
        ...
        #                                                      aug_mode=aug_mode, aug_num=aug_num)
        # self.logger.info("SMILES max len: {}".format(self.smi_max_len))

        if self.formula:
            self.logger.info("Dealing with input of SMILES...")
            self.smi_list, self.smi_max_len,formula_data = self.smiles2tensor(data["smiles"], smiles_vocab, smi_max_pad=smiles_max_pad,
                                                                 aug_mode=aug_mode, aug_num=aug_num, formula_data=list(data["formula"]))
            self.logger.info("SMILES max len: {}".format(self.smi_max_len))

            self.logger.info("Dealing with input of FORMULAS...")
            self.formula_list, self.formula_max_len = self.formula2tensor(formula_data, formula_vocab, formula_max_pad=formula_max_pad,
                                                                          aug_mode=aug_mode, aug_num=aug_num)
            ...
            self.logger.info("Dealing with input of SMILES...")
            self.smi_list, self.smi_max_len, formula_data = self.smiles2tensor(data["smiles"], smiles_vocab, smi_max_pad=smiles_max_pad,
                                                                 aug_mode=aug_mode, aug_num=aug_num, formula_data=None)
            self.logger.info("SMILES max len: {}".format(self.smi_max_len))

    def process_spec(self, spec_len, spec_data, aug_mode, aug_num, max_shift=None, theta=None, alpha=None):
        new_x = np.linspace(400, 3980, spec_len)
        if aug_mode == None:
            spec_list = [torch.tensor(interpolate_spectrum(i, new_x)[0]) for i in spec_data]

        elif aug_mode == 'verticalNoise':
    ...
    def vertical_noise(self, spec_x, spec_y, theta=0.01, alpha=1):
        """
        ...
        return interpolate_spectrum(aug_y, spec_x)[0]

    def shift_horizontal(self, spectrum, shift, new_x):
    ...
    def smiles2tensor(self, smi_data, vocab, smi_max_pad, aug_mode, aug_num, formula_data):
        new_lines = []
        max_len = 0
        # for smi in tqdm(smi_data):
        # skip_index = []
        for i,smi in enumerate(smi_data):
            len_list = []
            smi_ori = torch.tensor([ vocab[token] for token in split_smiles(smi)])
            if smi_ori.shape[0] > smi_max_pad: 
                self.logger.info("Skip {}: The tokens number of SMILES is larger than SMILES max pad({}).".format(smi, smi_max_pad))
                if smi_ori.shape[0] > max_len: max_len = smi_ori.shape[0]
                # skip_index.append(i)
                self.spec_list.pop(i)
                if self.formula: formula_data.pop(i)
                continue
            new_lines.append(smi_ori)
            ...
            if aug_mode =='SMILES':
                ...
                # for j in range(aug_num):
                    ...
                    if smi_j == smi or smi_j in aug_smi_list: continue

                    smi_j_ = torch.tensor([ vocab[token] for token in split_smiles(smi_j)])
                    if smi_j_.shape[0] > smi_max_pad: continue

                    aug_smi_list.append(smi_j)
                    new_lines.append(smi_j_)
                    len_list.append(smi_j_.shape[0])
                    j += 1
                    
            elif aug_mode == 'verticalNoise' or aug_mode == 'horizontalShift' or aug_mode=='horizontalShiftNonFP':
                new_lines += [smi_ori]*aug_num

            if max(len_list) > max_len:
                max_len = max(len_list)
            
        return new_lines, max_len, formula_data
    
    def formula2tensor(self, formula_data, vocab, formula_max_pad, aug_mode, aug_num):
        txt_split_list = [split_formula(i) for i in formula_data]
        new_lines = []
        max_len = 0
        # for line_ in tqdm(txt_split_list):
        for i,line_ in enumerate(txt_split_list):
            line = torch.tensor([ vocab[token] for token in line_])
            if line.shape[0] > formula_max_pad: 
                self.logger.info("Skip {}: The tokens number of formula is larger than formula max pad({}).".format(line_, formula_max_pad))
                if line.shape[0] > max_len: max_len = line.shape[0]
                self.spec_list.pop(i)
                self.smi_list.pop(i)
                continue
